...
- Storage keys are generated by the backend: `families/{familyId}/media/{mediaAssetId}/{upload|display|thumbnail}`. They are never derived from the file name, and never returned, logged or audited. A pre-signed URL necessarily carries its object's key in its path (S3 API); outside that short-lived URL, the key appears in no response field.
...
- The storage key is in no response field (only inside the pre-signed URL's path, §3.5), nor in the logs or the errors.
